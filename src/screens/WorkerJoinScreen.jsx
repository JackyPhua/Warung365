...
export default function WorkerJoinScreen({ onNavigate }) {
  ...
  const canScan = DispatchService.isNative
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        if (!active) return
        setStatus('❌ Camera/scan failed: ' + (e?.message || String(e)))
      }
    })()
  ...
  return (
    <div style={S.container}>
      <div style={S.header}>
        ...
        <div style={{ color: 'var(--primary)', fontSize: 16, fontWeight: 800 }}>📲 扫码入场</div>
        <div style={{ width: 70 }} />
      </div>
      ...
      <div style={S.scroll}>
        {!canScan && (
          <div style={S.warn}>
            ⚠️ 需要 APK（Android 原生）才能摄像头扫码。你现在在 Web 模式，可以把主机二维码内容复制/粘贴到下面。
          </div>
        )}

        <div style={S.section}>
          <div style={S.sectionTitle}>👤 工人名字（可选）</div>
          <input style={S.input} value={workerName} onChange={e => setWorkerName(e.target.value)} placeholder="Worker-01" />
        </div>

        <div style={S.section}>
          <div style={S.sectionTitle}>📷 摄像头扫码</div>
          <div style={{ color: 'var(--text-light)', fontSize: 12, marginBottom: 10 }}>
            对准主机屏幕二维码，识别后会自动填充 JSON。
          </div>
        ...
        <div style={S.section}>
          <div style={S.sectionTitle}>🧾 入场 JSON（可粘贴）</div>
          <textarea
            style={S.textarea}
            value={jsonText}
            ...
            rows={6}
          />
          <button style={S.primaryBtn} onClick={doJoin}>🔗 连接主机</button>
        </div>
      </div>
    </div>
  )
...
const S = {
  ...
  input: { width: '100%', background: 'var(--bg-input)', border: '1px solid var(--border)', borderRadius: 10, padding: '12px 14px', fontSize: 16, color: 'var(--text)', boxSizing: 'border-box' },
  videoBox: { width: '100%', aspectRatio: '16/10', background: '#000', borderRadius: 12, overflow: 'hidden', border: '1px solid var(--border)' },
